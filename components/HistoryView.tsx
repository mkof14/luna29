...
export const HistoryView: React.FC<{ log: HealthEvent[]; lang?: Language; onBack?: () => void }> = ({ log, lang = 'en', onBack }) => {
  ...
  const copyByLang: LangCopy<HistoryCopy> = {
    ...
    ar: { daily: 'تم حفظ التسجيل اليومي.', cycle: (day) => `تم تحديث الدورة إلى اليوم ${day}.`, health: 'تم تحديث بيانات الصحة.', support: (name) => `تم تحديث الدعم: ${name}.`, login: 'تم تسجيل الدخول.', started: 'بدأت.', profile: 'تم تحديث الملف الشخصي.', system: 'حدث نظام.', titleA: 'رحلتي', titleB: '.', subtitle: 'نظرة إلى رحلتك. كل إدخال جزء من قصتك.', noEntries: 'لا إدخالات بعد' },
    he: { daily: 'צ׳ק-אין יומי נשמר.', cycle: (day) => `המחזור עודכן ליום ${day}.`, health: 'נתוני בריאות עודכנו.', support: (name) => `התמיכה עודכנה: ${name}.`, login: 'התחברת.', started: 'התחיל.', profile: 'הפרופיל עודכן.', system: 'אירוע מערכת.', titleA: 'המסע', titleB: 'שלי.', subtitle: 'מבט לאחור על המסע שלך. כל רשומה היא חלק מהסיפור שלך.', noEntries: 'עדיין אין רשומות' },
  };
